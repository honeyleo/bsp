...
public class RedisBase{
	 ...
	 public class Sets{
		 ...
		 public List<String> srandMember(String key, int count) {
			 Jedis jedis = jedisPool.getResource();
			 List<String> members = jedis.srandmember(key, count);
			 return members;
		 }
	 }
	 ...
	 public class SortSet{
		 ...
		 /**
		  * 向集合中增加多条记录,如果这个值已存在，这个值对应的权重将被置为新的权重
		  * @param String key
		  * @param double score 权重
		  * @param String member 要加入的值，
		  * @return 状态码 1成功，0已存在member的值
		 ...
		 public long zinterstore(String dstkey, String... sets){
			 Jedis jedis = jedisPool.getResource();
			 long len = jedis.zinterstore(dstkey, sets);
			 jedisPool.returnResource(jedis);
			 return len;
		 }
		 ...
		 public Set<String> zrevrangeByScore(String key,double max,double min){
			 ShardedJedis sjedis=shardedJedisPool.getResource();
			 Set<String> set= sjedis.zrevrangeByScore(key, max, min);
			 shardedJedisPool.returnResource(sjedis);
			 return set;
		 }
		 ...
		 public Set<byte[]> zrevrange(byte[] key,int start,int end){
			 ShardedJedis sjedis=shardedJedisPool.getResource();
			 Set<byte[]> set= sjedis.zrevrange(key,start,end);
			 shardedJedisPool.returnResource(sjedis);
			 return set;
		 }
	 ...
	 public class Lists{
		 ...
		 public long lpush(String key,String... values){
			 return lpush(SafeEncoder.encode(key),SafeEncoder.encodeMany(values));
		 }
		  ...
		  * @return 记录总数
		  * */
		 public long lpush(byte[] key,final byte[]... values){
			 Jedis jedis=jedisPool.getResource();
